...
export class CommentsController {
    ...
    @UseGuards(AuthGuard('jwt'))
    @Patch('user/blog/:blogId/comment/:id')
    async updating(@Body() Text: string, @Param('blogId', ParseIntPipe) blogId: number, @Param('id', ParseIntPipe) id: number, @Req() req: any) {
        req.user = await this.userService.getTokenUser(req);
        const user1 = req.user
        return this.commentsService.UpdateComment(Text, id, blogId, user1.id);
    }
    @UseGuards(AuthGuard('jwt'))
